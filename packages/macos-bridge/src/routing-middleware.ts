...
import { detectUserAgent } from './detection.js';
import {
	OpenRouterHandler,
	GeminiHandler,
	OpenAIHandler,
...
import type { LogEntry, ApiKeys, DetectedApp } from './types.js';
...
export class RoutingMiddleware {
	...
	private createHandlerForModel(model: string): Handler {
		...
		if (model.startsWith('g/') || model.startsWith('gemini/')) {
			...
			return new GeminiHandler(geminiProvider, modelName, apiKey, this.bridgePort) as unknown as Handler;
		}
		...
		if (model.startsWith('oai/')) {
			...
			return new OpenAIHandler(openaiProvider, modelName, apiKey, this.bridgePort) as unknown as Handler;
		}
	...
	handle() {
		return async (c: Context, next: Next) => {
			...
			try {
				...
				// 2. Detect application from User-Agent
