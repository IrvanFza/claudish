...
import type { AccountInfo } from "../../providers/onepassword.js";
import type { FieldPickerOption, OpModalMode } from "../onepassword-fields.js";
...
/**
 * `OpModalMode`, `isOpModalMode`, `fuzzyMatch`, `fuzzyFilterByTitle`,
 * `FieldRowRole`, `FieldPickerOption` and `buildFieldOptions` MOVED to
 * `../onepassword-fields.ts`. They are pure functions over plain data with no
 * JSX in them, and `App.tsx`'s state layer calls them — so leaving them here
 * would force `hooks/` to import from `components/` the moment that state moves
 * into a hook. See that file's header for the full rationale.
 */
