...
- **`--probe --json` no longer prints your API keys.** `KeyProvenance.effectiveValue` held the resolved plaintext key and the whole record was serialized for every provider in a routing chain, so a bare-name probe emitted one key per credentialed candidate. The field is gone: the record now carries presence (`hasValue`), a display mask, the env-var NAME, and the source label — nothing value-derived. **If you have ever shared or pasted `--probe --json` output, rotate the keys it covered.**
...
- **Status-line chaining.** claudish no longer replaces a status line you already configured. It resolves the command Claude Code would have run — user settings → project → project local → `--settings`, matching Claude Code's own precedence — and appends its own segment to that output instead of clobbering it. Only provider, cost and context are appended; the directory and model name are dropped, because a custom line already shows both. The piped JSON payload is captured once and replayed, so a script that `jq`s it still works; multi-line output keeps its shape; ANSI passes through untouched; and failure is invisible (stderr dropped, empty capture falls back to claudish's segment alone). A 3s `timeout`/`gtimeout` guard is applied when either is on PATH. Only a `type: "command"` line can be chained — an `agent`-type or absent one keeps claudish's own.
- **Env contract for status lines.** Two new variables are published to anything running inside a claudish session: `CLAUDISH_TOKEN_FILE` (absolute path to this session's token file) and `CLAUDISH_PROVIDER_NAME` (provider display name; unset when unknown at spawn). A status line that sees either — claudish's own, or a chained user/plugin line such as magus statusline — is running against a proxied, non-Anthropic account and must not render Anthropic plan or rate-limit data.
- **Reasoning is now wire-aware.** A dialect is selected by model name, so on its own it cannot tell whether the same model is being reached over the OpenAI/Chat-Completions wire or the Anthropic Messages wire — and the reasoning knobs are named differently on each. The composed handler now passes the Layer 1 converter's stream format down to the dialect manager, so an Anthropic-wire request emits `thinking` + `output_config.effort` and an OpenAI-wire request keeps `reasoning_effort`. OpenAI-shaped fields (`reasoning_effort`, `enable_thinking`, `thinking_budget`) are stripped from Anthropic-wire requests, where they are silently ignored — measured against Qwen Plan, `reasoning_effort: "banana"` returns 200 there, so a dialect emitting it believed it set the depth and did nothing.
- **`zai@` reasoning turns on.** The GLM dialect had been deleting `thinking` on the Anthropic wire, which silently disabled reasoning for every `zai@` session. It is now translated rather than dropped.

### Bug Fixes

- **MiniMax thinking-filtering actually runs now.** `composed-handler.ts` handed the anthropic-sse parser the Layer 1 converter rather than the Layer 2 dialect, and every anthropic-sse provider passes an explicit `AnthropicAPIFormat` — so `shouldFilterThinking()` always answered the base default `false` and `MiniMaxModelDialect`'s `true` override had been unreachable dead code since it was written. The dialect now wins, matching the precedence already used for context window and vision. **Behaviour change: `mm@` / `mmc@` sessions stop showing raw thinking text.**
- **Reasoning effort is clamped to what the model actually supports.** An endpoint accepting a level is not the same as the model distinguishing it. GLM-5.2's endpoint accepts all seven canonical levels — `reasoning_effort: "banana"` returns a 400 listing every one — but [the model documents exactly two](https://docs.z.ai/guides/llm/glm-5.2), `max` and `high`, defaults to `max`, and treats **anything that is not `high` as Max**. Passing an unadvertised level straight through therefore inverted intent: a request for `low` is not `high`, so it ran at Max, and asking for less thinking bought the most. Requested levels are now clamped into the catalog's advertised set, ties resolving upward. Known data discrepancy, to be fixed in models-index rather than patched here: the slim catalog reports `["xhigh","high"]` with default `high` for glm-5.2 instead of `["max","high"]` with default `max` — harmless today only because anything ≠ `high` means Max.
- **Main-thread context window.** A `0` from `provider.getContextWindow()` means "no opinion" (OpenRouter's transport says so explicitly; the Codex transport returns it on a catalog miss), but it was applied unconditionally and overwrote the window the dialect had already resolved — so every OpenRouter-routed model reported `context_window: "unknown"` and lost the context field in the status line. `0` is now a no-op, and a still-unknown window falls back to a memoized single-model cloud catalog lookup off the hot path.

### Changes

- **Every model list is newest-first, by one shared comparator.** `compareByReleaseDateDesc` moved to `providers/model-ordering.ts` (model-loader and mcp-server both need it, and importing from model-selector would cycle and drag the inquirer picker into the MCP stdio path). It now orders discovered provider rosters — previously widest-window-first, which collapses to alphabetical on a plan whose entire roster is 1M and was what made the picker look unsorted — plus the recommended list and the local Ollama list. Curated ranking stays primary where it exists: `groupRecommendedModels` sorts by category-then-priority with freshness only as the tiebreak, and `search_models` keeps relevance primary with freshness breaking the many exact ties `fuzzyScore` produces (compared with an epsilon, since the scores are floats). `compare_models` deliberately keeps argument order.
- Freshness prefers the catalog's curated `releaseDate`, falling back to the provider endpoint's `created` timestamp. That fallback is a roster-added date, not a release date, but it is the only signal that exists for models the public catalog never listed — `qwen3.8-max-preview` is exactly that — and it is rejected outright unless it is a plausible seconds-scale value, so a millisecond timestamp cannot be read as the year 55000.
- The model picker no longer offers models you cannot chat with. A plan endpoint serves image and TTS models alongside chat ones, and they were all listed; the picker now filters with the SAME name-based predicate `--probe` uses — never a model-id skip list — so the two agree. Rows also carry the discovered context window and pricing. An all-non-chat roster falls through to free-text entry rather than showing an empty list.
